...
import com.app.itclao.imagetwo.Get_imagestwoshow;
...
public class PsporoductAdapter extends RecyclerView.Adapter<PsporoductAdapter.MyViewHolder> {
    private List<Product2> customerData;
    ...
    public PsporoductAdapter(Context context, List<Product2> customerData) {
    ...
    @NonNull
    @Override
    public PsporoductAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull final PsporoductAdapter.MyViewHolder holder, int position) {
       ...
       int bb = Integer.parseInt(qty);
        ...
        if (bb>aa){
            ...
            holder.check_qty.setText("ຈຳນວນສີນຄ້າ: "+bb);
       }else {
            holder.check_qty.setBackgroundColor(Color.parseColor("#FB4106"));
            holder.check_qty.setText("ຜະລິດຕະພັນ: "+qty);
        }
      ...
      if (img != null) {
        ...
        holder.card_product.setOnClickListener(new View.OnClickListener() {
            ...
            //   Toast.makeText(context, Customer_type.toString(), Toast.LENGTH_SHORT).show();
                player.start();
                Intent intent=new Intent(context, Get_imagestwoshow.class);
                ...
                if (cust.equals(0)){
                  ...
                  if (Status.equals("001")){
                      ...
                      //int check = databaseAccess.addToCart(sale_bill,sale_date,Tbname,product_id,product_name,price,cust.toString(),sale_status,edit_sale,username,Image,cut_qty);
                       int check = databaseAccess.additem_productorder(stock_id,product_id,s1_price,cust.toString(),bbaount,Product_Name,Unit,img);
                      databaseAccess.open();
                      int count=databaseAccess.getitem_productorder();
                      if (count==0) {
                          PsosActivity.txtCount.setVisibility(View.INVISIBLE);
                      } else{
                          PsosActivity. txtCount.setVisibility(View.VISIBLE);
                          PsosActivity.txtCount.setText(String.valueOf(count));
                      }
                      if (check == 1) {
                      ...
                      databaseAccess.open();
                      //int check = databaseAccess.addToCart(sale_bill,sale_date,Tbname,product_id,product_name,price,cust.toString(),sale_status,edit_sale,username,Image,cut_qty);
                      int check = databaseAccess.additem_productorder(stock_id,product_id,gets2_price,cust.toString(),bbaount,Product_Name,Unit,img);
                      databaseAccess.open();
                      int count=databaseAccess.getitem_productorder();
                      if (count==0) {
                          PsosActivity.txtCount.setVisibility(View.INVISIBLE);
                      ...
                      databaseAccess.open();
                      //int check = databaseAccess.addToCart(sale_bill,sale_date,Tbname,product_id,product_name,price,cust.toString(),sale_status,edit_sale,username,Image,cut_qty);
                      int check = databaseAccess.additem_productorder(stock_id,product_id,gets3_price,cust.toString(),bbaount,Product_Name,Unit,img);
                      databaseAccess.open();
                      int count=databaseAccess.getitem_productorder();
                      if (count==0) {
                          PsosActivity.txtCount.setVisibility(View.INVISIBLE);
