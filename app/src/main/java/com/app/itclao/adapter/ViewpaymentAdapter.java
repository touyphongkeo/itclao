...
public class ViewpaymentAdapter extends RecyclerView.Adapter<ViewpaymentAdapter.MyViewHolder> {


    Context context;
    private List<viewpaymoney> orderData;
    ...
    public ViewpaymentAdapter(Context context, List<viewpaymoney> orderData, TextView telphon, TextView usernaes, TextView idsssss, TextView text_v2, TextView text_v7, TextView text_v9) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        holder.txtProductName.setText(orderData.get(position).getsale_id());

        holder.txtProductPrice.setText(orderData.get(position).getcustomer_name());

     //   holder.txtTotalCost.setText(orderData.get(position).getpay_lak());
      ...
      //  String payment = orderData.get(0).getpay_lak();
       ...
       // text_v7.setText(f.format(billdiscouns)+" ກິບ");
        ...
        String amount = orderData.get(position).getpay_lak();
        double price = Double.parseDouble(amount);

        if (price==0){
            holder.txtTotalCost.setText("ຕິດໜີ້");
        }else {
